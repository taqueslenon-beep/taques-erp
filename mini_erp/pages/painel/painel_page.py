...
def painel():
    """
    Rota inicial do sistema - renderiza baseado no workspace atual.
    
    CORREÇÃO: Agora verifica o workspace atual e:
    - Se 'visao_geral_escritorio': redireciona para '/visao-geral/painel'
    - Se 'area_cliente_schmidmeier': renderiza painel da Área do Cliente
    """
    if not is_authenticated():
        ui.navigate.to('/login')
        return
    
    # Importa função para obter workspace atual
    from ...gerenciadores.gerenciador_workspace import obter_workspace_atual, definir_workspace
    
    # Obtém workspace atual
    workspace_atual = obter_workspace_atual()
    
    # Se for workspace "Visão Geral do Escritório", redireciona para rota específica
    if workspace_atual == 'visao_geral_escritorio':
        ui.navigate.to('/visao-geral/painel')
        return
    
    # Se for workspace "Área do Cliente", renderiza painel do cliente
    # Garante que o workspace está definido na sessão
    if workspace_atual == 'area_cliente_schmidmeier':
    ...
    # Fallback: se workspace desconhecido, redireciona para Painel
    ui.navigate.to('/visao-geral/painel')
    return
